...
| `Store` | `stores` | Merchant (name, address, geo) | → Receipt[] |
...
6. Normalise store name, find/create Store
...
- **System prompt**: Instructs extraction of store name, date, currency, amounts (cents), line items
- **Output format**: JSON with `store_name`, `transaction_date`, `currency`, `subtotal_cents`, `tax_cents`, `total_cents`, `line_items[]`
...
**Store names**: Known chain prefixes collapse to canonical names (Walmart, Costco, etc.). Others → `title()` case.
...
| `FUZZY_AUTO_LINK_THRESHOLD` | `85` | Auto-link score threshold |
| `FUZZY_SUGGEST_THRESHOLD` | `60` | Suggestion score threshold |
